<?php	
	$mk = '%'.$keyword.'%';
	$examinees = DB::table('examinees')->whereRaw('name LIKE "'.$mk.'" or exam_roll LIKE "'.$mk.'" or hsc_roll LIKE "'.$mk.'" or hsc_reg_no LIKE "'.$mk.'" or ssc_roll LIKE "'.$mk.'" or ssc_reg_no LIKE "'.$mk.'"')->get();
		...
		<div class="panel-body" style="padding: 0px;"></div>
			<?php
				if(!empty($examinees) and count($examinees) > 0)
				{
					...
					{
						echo '<a href="/result/'.$e->exam_roll.'" sl="'.$e->exam_roll.'" class="search_result_link list-group-item">';
						...
						echo '<span class="capsule unit-'.$e->unit.'">Unit '.$e->unit.'</span>';
						echo '<span class="capsule">Exam roll: '.$e->exam_roll.'</span>';
						echo '<span class="capsule">HSC roll: '.$e->hsc_roll.'</span>';
						echo '<span class="capsule">SSC roll: '.$e->ssc_roll.'</span>';
						echo '</div>';
